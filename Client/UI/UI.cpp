...
BattleUISys	bUI;		// User-user or Man-machine battle system UI
	...
	const char*, bool isBackup)> signupFunc)
{
	...
	ImGui::BeginGroup();
		...
		ImGui::TextWrapped("password:"); ImGui::SameLine(); ImGui::InputText("œ‘ æ√‹¬Î", password, sizeof(password),
			ImGuiInputTextFlags_Password | ImGuiInputTextFlags_CharsNoBlank);
		ImGui::SameLine(); showHelpMarker("(?)", password);

		ImGui::TextWrapped("validate:"); ImGui::SameLine(); ImGui::InputText("—È÷§√‹¬Î", validate, sizeof(validate),
			ImGuiInputTextFlags_Password | ImGuiInputTextFlags_CharsNoBlank);
...
void UChat::Draw(const char* title)
{
	ImGuiStyle& style = ImGui::GetStyle();
	style.WindowTitleAlign = ImVec2(0.50f, 0.50f);

	ImGui::SetNextWindowPos(ImVec2(0.0f, 0.0f));
	ImGui::SetNextWindowSize(ImVec2(370.0f, 1000.0f));
...
void FightUISys::DrawFightUI()
{

	// draw_list->AddCircleFilled(ImVec2(50.0f, 50.0f), 10.0f, ImColor(ImVec4(1.0f, 1.0f, 0.4f, 1.0f)));
	chatSys.Draw("Chat module(User-User)");
	...
	draw_list->AddRectFilled(ImVec2(770, 10), ImVec2(770 + 90, 10 + 90), ImColor(ImVec4(0.60f, 0.20f, 0.4f, 1.0f)), 10.0f);
	ImGui::NewLine(); ImGui::NewLine(); ImGui::SameLine(500);
	ImGui::TextWrapped("User's infos(?)"); ImGui::NewLine(); ImGui::SameLine(500);
	ImGui::TextWrapped("Wait:");
		...
		(MousePos.y >= (BLINE_UP_Y - LINE_SPACE / 2)) && (MousePos.y <= (BLINE_DOWN_Y + LINE_SPACE / 2)))
	{
		...
		// Draw white or black Chess
		static bool type = false;
		if (ImGui::IsMouseClicked(0))
		{
			if (!type)
			{
				chesses.push_back(CHESS(1, ImVec2(BLINE_LEFT_X + col * LINE_SPACE, BLINE_UP_Y + row * LINE_SPACE)));
				type = true;
			}
			else
			{
				chesses.push_back(CHESS(0, ImVec2(BLINE_LEFT_X + col * LINE_SPACE, BLINE_UP_Y + row * LINE_SPACE)));
				type = false;
			}
		}
